# Adapted from https://github.com/facebookresearch/hydra/blob/main/hydra/_internal/instantiate/_instantiate2.py
...
def instantiate(
    ...
    *args: Any,
    cache: Union[
        Dict[str, Any], None, Literal[True]
    ] = None,
    **kwargs: Any,
) -> Any:
    """
    :param config: An config object describing what to call and what params to use.
                   ...
                   _once_: If True, cache the result of the instantiation and reuse in instances.
    :param cache: Optional cache to use for _once_.
                  If True, use a persistent global cache.
                  If None (default), use an ephemeral cache for each call.
...
def _once_cache(cache = Union[None, Literal[True], Dict[str, Any]]) -> Dict[str, Any]:
...
def _once_hash(node, convert, recursive, partial):
    _once_ = node._once_
    if isinstance(_once_, str):
        return _once_

    Y = OmegaConf.to_yaml(node)
    if _Keys.CONVERT not in node:
        Y = f"{_Keys.CONVERT}: {convert}\n{Y}"
    if _Keys.RECURSIVE not in node:
        Y = f"{_Keys.RECURSIVE}: {recursive}\n{Y}"
    if _Keys.PARTIAL not in node:
        Y = f"{_Keys.PARTIAL}: {partial}\n{Y}"
    return Y
